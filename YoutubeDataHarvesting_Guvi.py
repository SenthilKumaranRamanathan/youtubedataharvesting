...
def videos_table():
    ...
    for vi_data in col1.find({}, {"_id": 0, "video_information": 1}):
        st.write(vi_data["video_information"][0:100])
...
st.markdown('''
        ...
        **Developed using :** App built in `Python` + `Streamlit` by [Senthilkumaran Ramanathan](GUVI [Data Engineer Program ])

        ---
        ''')

with st.sidebar:
    st.title(":blue[Web Application Youtube data Harvesting]")
    st.divider()
    st.caption(
        ":green[Effortlessly explore and analyze data from multiple channels, dive into the metrics that matter most to you, and experience the re-imagined world of YouTube analytics]:rocket:")
...
show_table = st.radio("SELECT THE TABLE BELOW TO VIEW",
                      (":black[Channels]", ":black[Playlists]", ":black[Videos]", ":black[Comments]"))
